...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'dev-sec-ops-cicd-pipeline-project-one', url: 'https://github.com/x0atsopze/DevSecOps-CICD-Pipeline-Microservice-Project.git'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit x0anderson/reddit:latest "
                       sh "docker push x0anderson/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image x0anderson/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@13.58.145.181 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://18.219.247.175:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#aat-multi-microservices-alerts', //update and provide your channel name
